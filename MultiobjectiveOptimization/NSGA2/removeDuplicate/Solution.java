...
//			newSol = VNS(newSol);
		...
		while(true){
			...
			for(int i = 0; i<combination.size();i++){
				for(int j = i+1; j<combination.size(); j++){
					if(combination.get(i).equals(combination.get(j))){
						repeatNum++;
					}
				}
			}
			...
			System.out.println(generationCount+"th generation: "+hyperVolume+" ranks: "+paretoFronts.size()+" #repeat: "+repeatNum);
			...
			// sheet.addCell(label);
			// label = new Label(1,count, paretoFronts.size()+"");
			// sheet.addCell(label);
			// count++;
	...
	public void generateChildren(){
		...
		while(childSet.size()<parentSize/2){
